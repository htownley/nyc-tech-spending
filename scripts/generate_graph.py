...
searchInput.addEventListener('input', () => {
  ...
  let searchSource;
  if (currentView === 'trends') {
    searchSource = Object.entries(TIMESERIES).map(([name, d]) => ({
      id: name, label: name, type: 'vendor', classification: d.classification,
      desc: d.description || '',
      spending: Object.values(d.yearly).reduce((a, b) => a + b, 0),
    }));
  } else {
    searchSource = RAW_NODES.map(n => ({
      id: n.id, label: n.id, type: n.type, classification: n.classification,
      desc: n.description || '', spending: n.spending,
    }));
    // Sankey: makers are searchable too (brand names like "Esri" live here,
    // while the vendor column uses legal payee names)
    if (currentView === 'sankey' && showMakers) {
      const mlKey = isOtiMode ? 'maker_links_oti' : 'maker_links';
      const totals = {};
      (PERIOD_DATA[currentPeriod][mlKey] || []).forEach(l => {
        totals[l.target] = (totals[l.target] || 0) + l.spend;
      });
      Object.entries(totals).forEach(([name, s]) => searchSource.push({
        id: 'maker::' + name, label: name, type: 'maker',
        classification: 'maker', desc: '', spending: s,
      }));
    }
  }
  // Match on display name first, description as fallback (catches brand
  // names inside legal-name vendors, e.g. "esri" → ENVIRONMENTAL SYSTEMS...)
  ...
  searchDropdown.innerHTML = matches.map(n => {
    ...
    const spendM = n.type !== 'agency' ? ` \u00b7 ${fmtUSD(n.spending)}` : '';
